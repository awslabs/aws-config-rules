...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if 'deadLetterConfig' not in configuration_item['configuration']:
        return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation='This Lambda function is not configured for DLQ')
    ...
    if 'deadLetterConfig' in configuration_item['configuration']:
        ...
        if configuration_item['configuration']['deadLetterConfig'] in valid_rule_parameters:
            ...
            return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation='This Lambda Function is not associated with the DLQ specified in the "dlqArn" input parameter.')
...
def evaluate_parameters(rule_parameters):
    ...
    for arn in dlqarn_list:
        if not (arn.startswith("arn:aws:sns:") or arn.startswith("arn:aws:sqs:")):
            raise ValueError(
                'Invalid value for the parameter "dlqArn", Expected Comma-separated list of valid SQS or SNS ARNs\'s')
