...
class EC2_INSTANCE_EBS_VOLUME_TAGS_MATCH(ConfigRule):
    def evaluate_change(self, event, client_factory, configuration_item, valid_rule_parameters):
        ...
        if resource_type == "AWS::EC2::Volume":
            ...
            instance_tags = get_resource_tags(config_client, instance_id, resource_type)
        ...
        if resource_type == "AWS::EC2::Instance":
            ...
            for volume_id in all_volumes:
                volume_tags = get_resource_tags(config_client, volume_id, resource_type)
                tags_to_apply = compare_instance_tags_to_volume_tags(instance_tags, volume_tags)
...
def get_resource_tags(config_client, resource_id, resource_type):
    sql = "SELECT resourceId, tags " f"WHERE resourceType = '{resource_type}' AND relationships.resourceId = '{resource_id}'"
    results = []

    query = config_client.select_resource_config(Expression=sql, Limit=CONFIG_PAGE_SIZE)
    next_token = True
    while next_token:
        results.extend(query["Results"])
        if "NextToken" in query:
            next_token = query.get("NextToken")
            sleep(CONFIG_THROTTLE_PERIOD)
            query = config_client.select_resource_config(Expression=sql, Limit=CONFIG_PAGE_SIZE, NextToken=next_token)
        else:
            next_token = False
