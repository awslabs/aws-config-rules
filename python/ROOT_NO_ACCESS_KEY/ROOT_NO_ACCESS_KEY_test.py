...
rule = __import__('ROOT_NO_ACCESS_KEY')

class ComplianceTest(unittest.TestCase):

    def test_access_keys_present(self):
        iam_client_mock.reset_mock()
        summary = {'SummaryMap': { 'AccountAccessKeysPresent': 1}}
        iam_client_mock.get_account_summary = MagicMock(return_value=summary)
        response = rule.lambda_handler(build_lambda_scheduled_event(),{})
        resp_expected = []
        resp_expected.append(build_expected_response('NON_COMPLIANT', '123456789012', annotation='The root user has access key(s).'))
        assert_successful_evaluation(self, response, resp_expected)

    def test_access_keys_not_present(self):
        iam_client_mock.reset_mock()
        summary = {'SummaryMap': { 'AccountAccessKeysPresent': 0}}
        iam_client_mock.get_account_summary = MagicMock(return_value=summary)
        response=rule.lambda_handler(build_lambda_scheduled_event(),{})
        resp_expected = []
