...
Rule Name:
    ConfigRule
Description:
    Checks if there are any orphan keys. The rule is NON_COMPLAINT for the orphan keys are found, and COMPLAINT for all the other keys. 
Trigger:
    Periodic
Reports on:
    AWS::KMS::Key
...
Scenarios:
        ...
        Given: At least 1 CMK is present
          And: The duplicate KMS Key is not an orphan key
         Then: Return COMPLIANT
        
        Scenario 3:
        Given: At least 1 CMK is present
          And: The duplicate KMS Key is an orphan key
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for alias in aliases : 
        if 'TargetKeyId' in alias:
            key_to_alias_dict[alias['TargetKeyId']] = alias['AliasName']

    for page in itr: 
        for key in page['Keys'] :
            list_of_keyids.append(key['KeyId'])
            try: 
                key_metadata = client.describe_key(KeyId = key['KeyId'])['KeyMetadata']
                date = key_metadata['CreationDate']
                modified_date = date.strftime("%D-%T")
                key_collapsed = {'keyId':key['KeyId'], 'alias':key_to_alias_dict.get(key['KeyId'],''), 'desc':key_metadata['Description']}
                if modified_date not in dict_duplicate_keys: 
                    dict_duplicate_keys[modified_date] = [key_collapsed]
                else : 
                    dict_duplicate_keys[modified_date].append(key_collapsed)
                    if dict_duplicate_keys[modified_date][0]['desc'] == dict_duplicate_keys[modified_date][1]['desc'] and (dict_duplicate_keys[modified_date][0]['alias'] == ''):
                        orphaned_keys.append(dict_duplicate_keys[modified_date][0]['keyId'])
                        non_orphaned_keys.append(dict_duplicate_keys[modified_date][1]['keyId'])
                    else: 
                        orphaned_keys.append(dict_duplicate_keys[modified_date][1]['keyId'])
                        non_orphaned_keys.append(dict_duplicate_keys[modified_date][0]['keyId'])
    ...
    if orphaned_keys != []:
        for key in orphaned_keys:
            evaluations.append(build_evaluation(key, 'NON_COMPLIANT', event,resource_type=DEFAULT_RESOURCE_TYPE, annotation = 'This KMS key is an orphan key'))

    if non_orphaned_keys != []:
        for key in non_orphaned_keys:
