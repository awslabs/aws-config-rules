...
#####################################
Rule Name:
  iam_user_without_character_string_test
Description:
  Verify that IAM user do not contain particular string.
Trigger:
  Event change
...
Rule Parameters:
  ...
  | ------------------- | --------- | -------------------------------------------------------- |
  | patternLst          | Required  | Find regex patterns in the IAM User Name.                |
  |                     |           | This action is triggered by an event change              |
  |                     |           |                                                          |
  | ------------------- | --------- | -------------------------------------------------------- |
Feature:
  In order to: enforce that cerain string patterns are not in IAM user names
  As: a Security Officer
  I want: To ensure that IAM users names do not the certain pharses eg: admin or power.
Scenarios:
  Scenario 1:
  Given: String Pattern
    And: A user name does not contain the string pattern
    Then: COMPLIANT
  Scenario 2:
  Given: String Pattern
    And: A user name does contain the string pattern
    Then: Return NON_COMPLIANT
  Scenario 3:
  Given: List of String Patterns
    And: A user name does contain the list of string patterns
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    pattern_lst = valid_rule_parameters["patternLst"]
    is_compliant = evaluateUsername(iam_user_name, pattern_lst)

    if is_compliant:
        evaluations.append(build_evaluation(iam_user_name, 'COMPLIANT', event))
    else:
        evaluations.append(build_evaluation(iam_user_name, 'NON_COMPLIANT', event))

    return evaluations

def evaluateUsername(user_name, pattern_lst):
    flag = True

    for pattern_str in pattern_lst:
...
def evaluate_parameters(rule_parameters):
    ...
    """
    valid_rule_parameters = json.loads(rule_parameters)
