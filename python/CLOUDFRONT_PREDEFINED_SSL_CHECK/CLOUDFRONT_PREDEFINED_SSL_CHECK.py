...
Rule Name:
  CLOUDFRONT_PREDEFINED_SSL_CHECK

Description:
  Checks whether your CloudFront Distribution are using a or several TLS/SSL predefined policy.
...
Feature:
  In order to: ensure that the traffic is encrypted in transit with the proper method
       ...
       I want: to verify that the configuration of any TLS/SSL policy of an CloudFront Distribution is correct.

Scenarios:
  Scenario 1:
     When: the CloudFront Distribution has not their TLS/SSL Policy name listed in the ValidPolicies parameter
     then: Return NON_COMPLIANT

  Scenario 2:
      When: the CloudFront Distribution has their TLS/SSL Policy name listed in the ValidPolicies parameter
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    """
    evaluations = []
    api_client = get_client('cloudfront', event)

    all_distributions = get_all_distributions(api_client)

    for distribution in all_distributions:
        https_bool, https_str = is_distribution_compliant(distribution, valid_rule_parameters)
        if not https_bool:
            evaluations.append(build_evaluation(distribution['Id'], 'NON_COMPLIANT', event, annotation=https_str))
            continue

        evaluations.append(build_evaluation(distribution['Id'], 'COMPLIANT', event))
...
def get_all_distributions(client):
    ...
    while resp:
        items += resp['Items']
        resp = client.describe_load_balancers(Marker=resp['NextMarker'])['DistributionList'] if 'NextMarker' in resp else None
    return items

def is_distribution_compliant(distribution, parameters):
    current_policy = distribution['ViewerCertificate']['MinimumProtocolVersion']
    if current_policy not in parameters['ValidPolicies']:
        return False, 'This CloudFront Distribution has a TLS/SSL policy ({}) not listed in the ValidPolicies parameter ({}).'.format(current_policy, ', '.join(parameters['ValidPolicies']))
    return True, None
...
def lambda_handler(event, context):
    ...
    AWS_CONFIG_CLIENT.put_evaluations(Evaluations=evaluations, ResultToken=result_token, TestMode=test_mode)
