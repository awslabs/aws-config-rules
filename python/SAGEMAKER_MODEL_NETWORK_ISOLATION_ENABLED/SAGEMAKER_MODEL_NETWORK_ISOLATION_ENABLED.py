...
Description:
  Check whether network isolation has been enabled for an Amazon SageMaker Model.
Trigger:
  Periodic
Reports on:
  AWS::::Account
Scenarios:
  Scenario: 1
     Given: No Amazon SageMaker models exist
      Then: Return NOT_APPLICABLE
  Scenario: 2
     Given: At least one Amazon SageMaker model exists
       And: Network isolation has been configured for the Amazon SageMaker model
      Then: Return COMPLIANT
  Scenario: 3
     Given: At least one Amazon SageMaker model exists
       And: Network Isolation is not enabled for the Amazon SageMaker model
      Then: Return NON_COMPLIANT with annotation "This Amazon SageMaker Model does not have network isolation enabled."
'''
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    model_list = get_all_models(sagemaker_client)
    #SCENARIO 1: No Amazon SageMaker models exist
    if not model_list:
        return None
    for model in model_list:
        model_description = sagemaker_client.describe_model(ModelName=model['ModelName'])
        #SCENARIO 2: Network isolation is enabled
        if model_description.get('EnableNetworkIsolation'):
            evaluations.append(build_evaluation(model['ModelName'], 'COMPLIANT', event))
        #SCENARIO 3: Network isolation is not enabled
        else:
            evaluations.append(build_evaluation(model['ModelName'], 'NON_COMPLIANT', event, annotation="This Amazon SageMaker Model does not have network isolation enabled."))
    return evaluations

#Returns a list of all the SageMaker models by making use of Boto3's pagination methods.
def get_all_models(sagemaker_client):
    model_list = []
