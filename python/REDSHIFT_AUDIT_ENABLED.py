...
def evaluate_compliance(event, configuration_item):
    ...
    for cluster_tuple in cluster_list:
        ...
        if cluster_tuple[1]['log_enabled'] == 'false':
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='Audit logging is not enforced for the cluster. Make sure to enable audit logging for the cluster. '))
        else:
            evaluations.append(
                build_evaluation(cluster_tuple[0], 'COMPLIANT', event, annotation='Audit logging is enabled'))
    return evaluations
