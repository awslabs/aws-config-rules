...
Description:
  Check whether use_fips_ssl is enabled for the RedShift cluster
Trigger:
  Periodic
Reports on:
...
Scenarios:
  Scenario: 1
     Given: No redshift cluster present
      Then: Return ERROR
   Scenario: 2
     Given: require_ssl is set to false
      Then: Return NON_COMPLIANT
   Scenario: 2
     Given: use_fips_ssl is set to false
      Then: Return NON_COMPLIANT
   Scenario 3:
    Given: If both use_fips_ssl and require_ssl are set to true
...
def evaluate_compliance(event, configuration_item):
    ...
    for cluster_tuple in cluster_list:
        # SCENARIO 2: SSL not enforced
        if cluster_tuple[1]['require_ssl'] == 'false':
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='SSL is not enforced for the cluster. Make sure to enable require_ssl by setting it to true prior to forcing FIPS.'))
        # SCENARIO 3: FIPS not enforced
        elif cluster_tuple[1]['use_fips_ssl'] == 'false':
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='FIPS is not enforced for the cluster. Make sure to enable require_ssl by setting it to true.'))
        else:
            evaluations.append(build_evaluation(cluster_tuple[0], 'COMPLIANT', event, annotation='FIPS is enforced'))
    return evaluations
