...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    ###############################
    budgets_client = get_client('budgets', event)
    budgets_list = get_all_budgets(budgets_client, event['accountId'])

    if not budgets_list:
        return build_evaluation(event["accountId"], 'NON_COMPLIANT', event, annotation='This Account has no AWS Budgets configured.')
...
def get_all_budgets(client, account_id):
    ...
    budgets = client.describe_budgets(AccountId=account_id)
    filter_aws_cost_budgets(budgets, budgets_list)

    while 'NextToken' in budgets:
        budgets = client.describe_budgets(AccountId=account_id, NextToken=budgets['NextToken'])
        filter_aws_cost_budgets(budgets, budgets_list)

    return budgets_list


def filter_aws_cost_budgets(budgets, budgets_list):
...
def lambda_handler(event, context):
    ...
    if not compliance_result:
        latest_evaluations.append(
            build_evaluation(event["accountId"], "NOT_APPLICABLE", event, resource_type=DEFAULT_RESOURCE_TYPE)
        )
