...
def lambda_handler(event, context):
    ...
    glue_job_name = event['ResourceId']
    glue_client = boto3.client('glue')
    
    configuration_name = get_job_security_configuartion(glue_job_name, glue_client)
    if not configuration_name:
        logging.info('Glue job has no security configuration, creating a new one now')
        new_security_configuration = create_security_configuration(glue_client, glue_job_name)
        change_job_security_configuartion(glue_client, glue_job_name, new_security_configuration)
        ...
        if s3_encryption_config[next(iter(s3_encryption_config))] == 'DISABLED' or cloudwatch_encryption_config[next(iter(cloudwatch_encryption_config))] == 'DISABLED':
            new_security_configuration = create_security_configuration(glue_client, glue_job_name)
            change_job_security_configuartion(glue_client, glue_job_name, new_security_configuration)
            logging.info(glue_job_name + "'s security configuration has been updated to a new one called " + new_security_configuration)
...
def change_job_security_configuartion(glue_client, glue_job_name, new_security_configuration):
    ...
    response = glue_client.get_job(JobName=glue_job_name)['Job']
    job_role = response['Role']
    job_command = response['Command']
...
def get_job_security_configuartion(glue_job_name, glue_client):
...
def create_security_configuration(glue_client, glue_job_name):
    """This function creates a new encrypted security configuration"""
    
    kms_key_arn = create_kms_key()
    security_config_name = glue_job_name + '_SecurityConfiguration'
    try: 
