...
Scenarios:
  Scenario: 1
   ...
   Scenario 5:
     Given: The parameter status of use_fips_ssl and require_ssl  is not synced
       Then: Return NON_COMPLIANT
"""
...
def evaluate_compliance(event, configuration_item):
    evaluations = []
    redshift_client = get_client("redshift", event)
        ...
        if cluster_tuple[1]["require_ssl"] == False:
            evaluations.append(
                build_evaluation(
                    ...
                    annotation="SSL is not enforced for the cluster. Make sure to enable require_ssl by "
                    "setting it to true prior to forcing FIPS and ensure the parameters "
                    "are in sync.",
                )
            )
        # SCENARIO 3: FIPS not enforced
        elif not cluster_tuple[1]["use_fips_ssl"]:
            evaluations.append(
                build_evaluation(
                    cluster_tuple[0],
                    "NON_COMPLIANT",
                    event,
                    annotation="FIPS is not enforced for the cluster. Make sure to enable "
                    "require_ssl by setting it to true.",
                )
            )
        elif cluster_tuple[1]["use_fips_ssl"]:
            evaluations.append(
                build_evaluation(
                    cluster_tuple[0], "COMPLIANT",event
                )
            )
        else:
