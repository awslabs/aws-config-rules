...
class ComplianceTest(unittest.TestCase):
    ...
    def test_log_group_retention_set_to_never_expire(self):
        LOGS_CLIENT_MOCK.describe_log_groups = MagicMock(return_value=self.log_group_retention_set_to_never_expire)
        RULE.ASSUME_ROLE_MODE = False
        response = RULE.lambda_handler(build_lambda_scheduled_event(self.rule_empty_parameter), {})
        resp_expected = []
        resp_expected.append(build_expected_response('NON_COMPLIANT', '/aws/lambda/ALBLambda', 'AWS::Logs::LogGroup', 'This CloudWatch Log Group has a retention period set to Never Expire'))
        assert_successful_evaluation(self, response, resp_expected)

    # Scenario 3
    def test_log_group_retention_set_in_days(self):
        LOGS_CLIENT_MOCK.describe_log_groups = MagicMock(return_value=self.log_group_retention_set_in_days)
        RULE.ASSUME_ROLE_MODE = False
        response = RULE.lambda_handler(build_lambda_scheduled_event(self.rule_empty_parameter), {})
        resp_expected = []
        resp_expected.append(build_expected_response('COMPLIANT', '/aws/lambda/ALBLambda', 'AWS::Logs::LogGroup', 'This CloudWatch Log Group has a retention period set to 60 days'))
        assert_successful_evaluation(self, response, resp_expected)
