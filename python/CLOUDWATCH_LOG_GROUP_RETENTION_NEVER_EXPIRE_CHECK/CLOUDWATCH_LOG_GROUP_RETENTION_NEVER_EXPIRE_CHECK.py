...
#####################################
Rule Name:
    CLOUDWATCH_LOG_GROUP_RETENTION_NEVER_EXPIRE_CHECK
Description:
...
Scenarios:
  ...
  Scenario: 2
    ...
    And: Log Group has a retention period of 'Never Expire' set (DescribeLogGroups call does not return the 'retentionInDays' name/value pair).
    Then: Return NON_COMPLIANT
  Scenario: 3
    Given: At least one CloudWatch Log Group is present.
    And: Log Group has a retention period set (DescribeLogGroups call returns the 'retentionInDays' name/value pair).
