...
Scenarios:
  ...
  Scenario: 2
   ...
   Scenario: 3
     Given: enable_user_activity_loggingis not enabled
      Then: Return NON_COMPLIANT
   Scenario 4:
     Given: If Audit logging and enable_user_activity_logging are both enabled
      Then: Return COMPLIANT
   Scenario 5:
     Given: The parameter status of enable_user_activity_logging is not synced
       Then: Return NON_COMPLIANT
"""
        ...
        if not cluster_tuple[1]["log_enabled"]:
            evaluations.append(
                build_evaluation(
                    ...
                    "NON_COMPLIANT",
                    event,
                    annotation="Audit logging is not enforced for the cluster. Make sure to enable audit logging for "
                               "the cluster. Without audit logging enabled, you cannot track user activity.",
                )
            )
        # SCENARIO 3: User activity logging is not enabled
            ...
            evaluations.append(
                build_evaluation(
                    ...
                    "NON_COMPLIANT",
                    event,
                    annotation="enable_user_activity_logging is not enforced for the cluster. "
                               "Make sure to enable enable_user_activity_logging by setting it to true and"
