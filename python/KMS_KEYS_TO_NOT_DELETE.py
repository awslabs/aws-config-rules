...
Scenarios:
	...
	And: The KMS Key is not one of the keys in the parameter
	Then: Return NOT_APPLICABLE
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    account_keys = get_account_keys(kms_client)
    ...
    if 'kmsKeyIds' in valid_rule_parameters:
        for key_id in valid_rule_parameters['kmsKeyIds']:
            if key_id in account_keys:
                kms_key_details = kms_client.describe_key(KeyId=key_id)
                if kms_key_details['KeyMetadata']['Origin'] == 'AWS_KMS' and kms_key_details['KeyMetadata']['KeyManager'] == 'CUSTOMER' and kms_key_details['KeyMetadata']['KeyState'] == 'PendingDeletion':
                    evaluations.append(build_evaluation(key_id, 'NON_COMPLIANT', event, DEFAULT_RESOURCE_TYPE, annotation='The KMS Key is scheduled for deletion.'))
                    continue
                evaluations.append(build_evaluation(key_id, 'COMPLIANT', event))
        return evaluations

    for key_id in account_keys:
        kms_key_details = kms_client.describe_key(KeyId=key_id)
        if kms_key_details['KeyMetadata']['Origin'] == 'AWS_KMS' and kms_key_details['KeyMetadata']['KeyManager'] == 'CUSTOMER' and kms_key_details['KeyMetadata']['KeyState'] == 'PendingDeletion':
            evaluations.append(build_evaluation(key_id, 'NON_COMPLIANT', event, DEFAULT_RESOURCE_TYPE, annotation='The KMS Key is scheduled for deletion.'))
            continue
...
def get_account_keys(kms_client):
    account_key_library = []
    response = kms_client.list_keys()

    while response['Keys']:
        for key in response['Keys']:
            account_key_library.append(key['KeyId'])
        if not 'NextMarker' in response:
            return account_key_library
