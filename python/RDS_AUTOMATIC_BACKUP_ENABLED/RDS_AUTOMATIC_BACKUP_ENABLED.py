...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    #Get backupRetentionPeriod from CI
    backup_retention_period = configuration_item['configuration']['backupRetentionPeriod']

    if backup_retention_period>0:
        #Scenario 1: backupRetentionPeriod > 0, this means Automated backups are enabled
        return build_evaluation_from_config_item(configuration_item, 'COMPLIANT')
    else:
        #Scenario 2: backupRetentionPeriod = 0, this means Automated backups are disabled
        return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation='Automated Backups are Disabled for this RDS Instance.')
