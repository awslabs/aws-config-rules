...
Rule Name:
  instance-profile-have-defined-policies

Description:
  Checks that the correct policies are attached to particular EC2 instance profiles.

Trigger:
  Configuration Changes on AWS::IAM::Role
...
Feature:
  In order to: ensure the proper permissions for logging, backup and others,
  As: a Security Officer
  I want: To ensure that Instance Profile policies match our expectations.

Scenarios:

  Scenario 1:
    Given: An IAM role name which does not includes NAME_ROLE_TO_CHECK.
     Then: Return NOT_APPLICABLE

  Scenario 2:
    Given: An IAM role name which includes NAME_ROLE_TO_CHECK.
      And: At least one policy in POLICY_NAMES_THAT_MUST_BE_ATTACHED is not attached.
     Then: Return NON_COMPLIANT

  Scenario 3:
    Given: An IAM role name which includes NAME_ROLE_TO_CHECK.
      And: All policies in POLICY_NAMES_THAT_MUST_BE_ATTACHED are attached.
...
NAME_ROLE_TO_CHECK = '.db.ec2role.iaminstancerole'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if resource_name.find(NAME_ROLE_TO_CHECK) == -1:
        return 'NOT_APPLICABLE'

    policy_counter = 0
    for policy in configuration_item['configuration']['attachedManagedPolicies']:
        if policy['policyName'] in POLICY_NAMES_THAT_MUST_BE_ATTACHED:
            policy_counter += 1
        if policy_counter == len(POLICY_NAMES_THAT_MUST_BE_ATTACHED):
            return 'COMPLIANT'

    return build_evaluation_from_config_item(configuration_item, 'NON_COMPLIANT', annotation="One or more mandatory policy is not attached to this instance profile.")
