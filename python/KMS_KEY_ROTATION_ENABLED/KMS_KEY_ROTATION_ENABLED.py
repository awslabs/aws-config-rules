...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    all_aliases = get_all_aliases(kms_client)
    aliases = []
    ...
    for key in keys:
        ...
        if not key_metadata['Enabled']:
            ...
            alias_dict = next((alias for alias in aliases if alias['TargetKeyId'] == key['KeyId']), None)
            if alias_dict:
                key_alias = alias_dict['AliasName'].split("/")[1]
                annotation = 'The yearly rotation is not activated for key with alias "' + key_alias + '".'
            else:
                annotation = 'The yearly rotation is not activated for key with ID "' + key['KeyId'] + '".'
            eval_key = build_evaluation(key['KeyId'], "NON_COMPLIANT", event, annotation=annotation)
        evaluations.append(eval_key)
    return evaluations

def get_all_aliases(kms_client):
    aliases = kms_client.list_aliases(Limit=100)
    all_aliases = []
    while True:
        all_aliases += aliases['Aliases']
        if "NextMarker" in aliases:
            aliases = kms_client.list_aliases(Marker=aliases["NextMarker"], Limit=100)
        else:
            break
    return all_aliases
