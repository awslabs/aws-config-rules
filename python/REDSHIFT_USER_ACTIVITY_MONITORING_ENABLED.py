...
Description:
  Check whether require_ssl is enabled for the RedShift cluster
Trigger:
  Periodic
Reports on:
...
Scenarios:
  ...
  Scenario: 2
   ...
   Scenario: 3
     Given: User activity logging is not enabled
      Then: Return NON_COMPLIANT
   Scenario 4:
     Given: If Audit logging and user activity logging are both enabled
...
def get_logging_status(cluster_id):
...
def get_parameters_for_cluster_parameter_group(cluster_parameter_group):
    ...
    for param in parameters:
        if param['ParameterName'] in params_to_scan:
            if param['ParameterValue'] == 'true' and param['ParameterName'] not in sync_list:
...
def get_cluster_list():
    ...
    for cluster in clusters:
        params = get_parameters_for_cluster_parameter_group(cluster['ClusterParameterGroups'])
        ...
        params.update({'log_enabled': get_logging_status(cluster['ClusterIdentifier'])})
        cluster_tuple_list.append((cluster['ClusterIdentifier'], params,))
...
def evaluate_compliance(event, configuration_item):
    global CLIENT
    evaluations = []
    CLIENT = get_client('redshift', event)
    ...
    for cluster_tuple in cluster_list:
        ...
        if not cluster_tuple[1]['log_enabled']:
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='Audit logging is not enforced for the cluster. Make sure to enable audit logging for the cluster. Without audit logging enabled, you cannot track user activity.'))
        # SCENARIO 3: User activity logging is not enabled
        elif not cluster_tuple[1]['enable_user_activity_logging']:
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='enable_user_activity_logging is not enforced for the cluster. Make sure to enable enable_user_activity_logging by setting it to true.'))
        else:
            evaluations.append(build_evaluation(cluster_tuple[0], 'COMPLIANT', event,
                                                annotation='enable_user_activity_logging is enforced and audit logging is turned on'))
    return evaluations
