...
#   ECS_AWSLOGS_CHECK
# Description:
#   Check ECS Task Definitions for presence of awslogs driver
#
# Rationale:
#   ECS Task Definitions should be configured with awslogs driver enabled for logging purpose.
#
# Indicative Severity:
#   Medium
#
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    taskdefs = []
    compliant = []
    loggingdriver = re.compile("com.amazonaws.ecs.capability.logging-driver.awslogs")
    ...
    if valid_rule_parameters:
        ...
        for taskdef in taskdefs:
            ...
            for attribute in response["taskDefinition"]["requiresAttributes"]:
                if loggingdriver.search(attribute["name"]):
                    evaluations.append(build_evaluation(taskdef, 'COMPLIANT', event, annotation='Task Definition has awslogs log driver enabled.'))
                    compliant.append(taskdef)
                    break
            if taskdef not in compliant:
                evaluations.append(build_evaluation(taskdef, 'NON_COMPLIANT', event, annotation='Task Definition does not have awslogs log driver enabled.'))
    return evaluations
