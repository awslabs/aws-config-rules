...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if invoking_event["configurationItem"]["resourceType"] == DEFAULT_RESOURCE_TYPE:
        ...
        # init
        resource_tags = dict()
        resource_tags["ENIs"] = dict()
        resource_tags["Volumes"] = dict()
...
def evaluate_parameters(rule_parameters):
    ...
    """
    valid_parameters = ["TagName", "VPC", "ENIs", "Subnet", "Volumes", "SecurityGroups"]
    for parameter in rule_parameters:
        if parameter not in valid_parameters:
            raise ValueError("Following parameter is invalid: " + parameter)
