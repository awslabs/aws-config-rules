...
#####################################
Rule Name:
  SSM_PATCHES_SUCCEEDED_STATE
Description:
  Verify patches were applied successfully.
Trigger:
  Scheduled change on AWS::EC2::Instance
Resource Type to report on:
  AWS::EC2::Instance
Scenarios:
  Scenario 1:
  Given: No patches are in a missing or failed state
    Then: COMPLIANT
  Scenario 2:
  Given: Patch state is installed or missing
    Then: NON_COMPLIANT
  Scenario 3:
  Given: Patch state is installed other or failed state
    Then: NON_COMPLIANT
'''
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    instance_id = json.loads(event["invokingEvent"])["configurationItem"]["resourceId"]
    ...
    for patch in response["Patches"]:
        ...
        if state in ["MISSING", "FAILED"]:
            return build_evaluation(instance_id, 'NON_COMPLIANT', event, annotation='The patch is missing or failed ({}) on the following instance ({}).'.format(state, instance_id))

    return build_evaluation(instance_id, 'COMPLIANT', event)
...
def build_evaluation(resource_id, compliance_type, event, resource_type=DEFAULT_RESOURCE_TYPE, annotation=None):
    ...
    event -- the event variable given in the lambda handler
    resource_type -- the CloudFormation resource type (or AWSAWS::EC2::Instance) to report on the rule (default DEFAULT_RESOURCE_TYPE)
...
# This removes older evaluation (usually useful for periodic rule not reporting on AWS::EC2::Instance).
...
def lambda_handler(event, context):
    ...
    if not compliance_result:
        latest_evaluations.append(build_evaluation(event['accountId'], "NOT_APPLICABLE", event, resource_type='AWS::EC2::Instance'))
