...
#
# Ensure DataProtection Policy is attached to CloudWatch Log Groups
# Description: Checks that DataProtection Policy is attached to CloudWatch Log Groups
#
# Trigger Type: Periodic
# Scope of Changes: AWS::Logs::LogGroup
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    #print(all_log_groups)
    # No log group exists
    if not all_log_groups:
        return None

    for each_loggroup in all_log_groups:
        # NON_COMPLIANT if dataProtectionStatus does not exist.
        if "dataProtectionStatus" not in each_loggroup:
            evaluations.append(build_evaluation(each_loggroup['logGroupName'], 'NON_COMPLIANT', event, annotation='This CloudWatch Log Group is not protecting sensitve data'))
            continue
        #if dataProtectionStatus is 'ACTIVATED' then set status as compliant.
        if each_loggroup['dataProtectionStatus'] == 'ACTIVATED':
...
def get_client(service, event):
    ...
    credentials = get_assume_role_credentials(get_execution_role_arn(event))
...
def lambda_handler(event, context):
    ...
    # Invoke the Config API to report the result of the evaluationFassu
    evaluation_copy = []
