...
'''
  ...
  #####################################  
  Rule Name:
  BUSINESS_SUPPORT_ENABLED
  
  Description:
  Check whether the AWS Account has Business support plans enabled.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        ...
        if error.response['Error']['Code'] == 'SubscriptionRequiredException':
            ...
            annotate = 'Role does not have permissions to use AWS Support API'
            rule_evaluations.append(build_evaluation(account_id, 'NON_COMPLIANT', event, annotation=annotate))
        else:
            annotate = 'Unexpected error. Please check CloudWatch logs for this rule. '
            print(error)
            rule_evaluations.append(build_evaluation(account_id, 'NOT_APPLICABLE', event, annotation=annotate))
