...
Description:
  Check whether direct internet access is disabled for an Amazon SageMaker Notebook Instance.
...
Scenarios:
  ...
  Scenario: 2
     Given: At least one Amazon SageMaker notebook instance exists
       And: DirectInternetAccess is set to Enabled for the Amazon SageMaker notebook instance
      Then: Return NON_COMPLIANT with annotation "This Amazon SageMaker Notebook Instance has direct internet access."
  Scenario: 3
     Given: At least one Amazon SageMaker notebook instance exists
       And: DirectInternetAccess is set to Disabled for the Amazon SageMaker notebook instance
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for notebook_instance in notebook_instance_list:
        ...
        #SCENARIO 2: DirectInternetAccess is set to Enabled for the Amazon SageMaker notebook instance
        if notebook_instance_description['DirectInternetAccess'] == 'Enabled':
            evaluations.append(build_evaluation(notebook_instance['NotebookInstanceName'], 'NON_COMPLIANT', event, annotation="This Amazon SageMaker Notebook Instance has direct internet access."))
        #SCENARIO 3: DirectInternetAccess is set to Disabled for the Amazon SageMaker notebook instance
        else:
...
def get_all_notebook_instances(sagemaker_client):
    ...
    for page in page_iterator:
        for notebook_instance in page['NotebookInstances']:
            notebook_instance_list.append(notebook_instance)
    return notebook_instance_list
