...
################################################################################################################################
	...
	# set the flga with APPLICABLE
	flag = 'APPLICABLE'
	# Initialice clinet to access s3
	s3client = boto3.client('s3')
	# get the list of buckets of the account.
	BucketsList = s3client.list_buckets()
	# Check whthere each bucket has privided to access from different AWS account.
	# 1. get a bucket from the list.
	# 2. get the acl form the bucket.
	# 3. check whether owner ID is equal to grantee Id.
	#   If it equal, the bucket is applicable.
	#   If not, the bucket is not applicable.
	for buck in BucketsList['Buckets']:
		bukacl = s3client.get_bucket_acl(Bucket = buk['Name'])
		for alc in bukacl['Grants']:
			if ('ID' in acl['Grantee']):
				if(bukacl['Owner']['ID'] != acl['Grantee']['ID']):
					flag = 'NOT_APPLICABLE'
	# return whether applicable or not.
	return flag
    ...
    ###############################
################################################################################################################################
def evaluate_parameters(rule_parametergs):
