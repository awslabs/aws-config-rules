...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if configuration_item['configuration']['distributionConfig']['defaultCacheBehavior']['viewerProtocolPolicy'] == 'allow-all':
        return build_evaluation(distribution_id, 'NON_COMPLIANT', event, DEFAULT_RESOURCE_TYPE, annotation='''Default ViewerProtocolPolicy is set to 'allow-all' for this distribution.''')
    if configuration_item['configuration']['distributionConfig']['cacheBehaviors']['quantity'] > 0:
        for behavior in configuration_item['configuration']['distributionConfig']['cacheBehaviors']['items']:
            ...
            if behavior['viewerProtocolPolicy'] == 'allow-all':
                return build_evaluation(distribution_id, 'NON_COMPLIANT', event, DEFAULT_RESOURCE_TYPE, annotation='''ViewerProtocolPolicy for path "{}" is set to 'allow-all.' '''.format(path))
