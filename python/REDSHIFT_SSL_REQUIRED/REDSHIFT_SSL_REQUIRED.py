...
Scenarios:
  ...
  Scenario: 2
   ...
   Scenario 4:
     Given: The parameter status of require_fips_ssl  is not synced
       Then: Return NON_COMPLIANT
"""
...
def evaluate_compliance(event, configuration_item):
    ...
    for cluster_tuple in cluster_list:
        ...
        if not cluster_tuple[1]["require_ssl"]:
            evaluations.append(
                build_evaluation(
                    cluster_tuple[0],
                    "NON_COMPLIANT",
                    event,
                    annotation="SSL/TLS is not enforced for the cluster. Make sure to enable require_ssl by setting it "
                               "to true and ensure the parameters are synced.",
                )
            )
        elif cluster_tuple[1]["require_ssl"]:
            evaluations.append(
                build_evaluation(
                    cluster_tuple[0],
                    "COMPLIANT",
