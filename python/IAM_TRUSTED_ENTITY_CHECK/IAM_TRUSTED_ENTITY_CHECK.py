...
"""
Description:
  Checks whether trusted entities in an IAM Role includes a rule which trusts whole of another AWS Account.
Trigger:
  Periodic
Reports on:
  AWS::IAM::Role
Rule Parameters:
  None
Scenario: 1
     Given: The Role has a rule which matches '^arn:aws:iam::[0-9]{12}:root$'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    """
    evaluations = []
    for role in generate_iam_role(event):
        # print(role)
        ...
        for statement in policy_doc['Statement']:
            if 'AWS' in statement['Principal']:
                ...
                for principal in principals:
                    # check principle
                    # print(principal, is_another_aws_account_trusted(principal))
                    result_list.append(
                        is_another_aws_account_trusted(principal)
                    )
                
            else:
                # If statement doesn't have 'AWS' key, result is treated as COMPLIANT
                result_list.append(False)
        if any(result_list):
            # If any of the results is True, the resource includes principal which trusts whole of another AWS Account.
            evaluations.append(
                build_evaluation(role['RoleId'], 'NON_COMPLIANT', event)
            )
        else:
            evaluations.append(
                build_evaluation(role['RoleId'], 'COMPLIANT', event)
            )

    return evaluations


def generate_iam_role(event):
    iam = get_client('iam', event, 'ap-northeast-1')
    res = iam.list_roles()
    for role in res['Roles']:
        yield role


def is_another_aws_account_trusted(principle):
    m = re.match(r'^arn:aws:iam::[0-9]{12}:root$', principle)
    if not m:
        return False
    return True
