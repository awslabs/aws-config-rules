...
Scenarios:
  Scenario 1:
  Given: No Amazon EMR cluster is in RUNNING or WAITING state
   Then: Return NOT_APPLICABLE
  Scenario 2:
  Given: At least 1 Amazon EMR cluster is in RUNNING or WAITING state
    And: At least one of the security groups attached to the Amazon EMR cluster has a rule with the IPv4 range '0.0.0.0/0' or the IPv6 range '::/0'
   Then: Return NON_COMPLIANT on this cluster
  Scenario 3:
  Given: At least 1 Amazon EMR cluster is in RUNNING or WAITING state
    And: None of the security groups attached to the Amazon EMR cluster have a rule with the IPv4 range '0.0.0.0/0' or the IPv6 range '::/0'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    cluster_list = paginated_list_clusters(emr_client)
    if not cluster_list:
        return None
    ...
    for cluster in cluster_list:
        ...
        if cluster["Id"] in cluster_open_sg_list:
            evaluations.append(build_evaluation(cluster["Id"], 'NON_COMPLIANT', event, annotation="This Amazon EMR cluster has one or more Security Groups open to the world."))
        #Scenario 3: No security group for the Amazon EMR cluster has a rule with IP range 0.0.0.0/0 or ::/0
        else:
            evaluations.append(build_evaluation(cluster["Id"], 'COMPLIANT', event))
    return evaluations

def paginated_list_clusters(emr_client):
    cluster_list = []
    paginator = emr_client.get_paginator('list_clusters')
    page_iterator = paginator.paginate(ClusterStates=['WAITING', 'RUNNING'])
    for page in page_iterator:
        for cluster in page['Clusters']:
            cluster_list.append(cluster)
    return cluster_list

def get_sg_cluster_mapping(cluster_list, emr_client):
    sg_cluster_map = {}
    sg_list_combined = set([])
    for cluster in cluster_list:
        sg_list = set([])
        cluster_id = cluster["Id"]
        cluster_description = emr_client.describe_cluster(ClusterId=cluster_id)
        sg_list.add(cluster_description["Cluster"]["Ec2InstanceAttributes"]["EmrManagedSlaveSecurityGroup"])
        sg_list.add(cluster_description["Cluster"]["Ec2InstanceAttributes"]["EmrManagedMasterSecurityGroup"])
        sg_list.update(cluster_description["Cluster"]["Ec2InstanceAttributes"]["AdditionalSlaveSecurityGroups"])
        sg_list.update(cluster_description["Cluster"]["Ec2InstanceAttributes"]["AdditionalMasterSecurityGroups"])
        for security_group in sg_list:
            if security_group in sg_list_combined:
                sg_cluster_map[security_group].append(cluster_id)
            else:
                sg_cluster_map[security_group] = []
                sg_cluster_map[security_group].append(cluster_id)
