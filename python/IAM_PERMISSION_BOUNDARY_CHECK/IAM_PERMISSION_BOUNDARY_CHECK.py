...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if(configuration_item):
        ...
        return evaluate_user(event, username,valid_rule_parameters)
    else:
        iam_client = get_client('iam', event)
        users_list = iam_client.list_users()
        permission_boudary_list = iam_client.list_policies(OnlyAttached=True,PolicyUsageFilter='PermissionsBoundary')
        if(users_list and permission_boudary_list):
...
def evaluate_user(event, username,valid_rule_parameters):
    ...
    try:
        if(user_details['User']['PermissionsBoundary']):
            try:
                if(valid_rule_parameters['PermissionBoundaryName']):
                    permission_boundary_policy_names = valid_rule_parameters['PermissionBoundaryName'].replace(" ","")
                    permission_boundary_policy_name_list = permission_boundary_policy_names.split(",")
                    boundry_name=user_details['User']['PermissionsBoundary']['PermissionsBoundaryArn'].split('/')
                    boundry_name=boundry_name[-1]
                    for permission_policy_name in permission_boundary_policy_name_list:
                        if(permission_policy_name == boundry_name):
...
def evaluate_parameters(rule_parameters):
    ...
    if(valid_rule_parameters):
        ...
        permission_boundary_policy_name_list = permission_boundary_policy_names.split(",")
        for permission_policy_name in permission_boundary_policy_name_list:
            if('/' in permission_policy_name or '\\' in permission_policy_name or '*' in permission_policy_name or '?' in permission_policy_name):
                raise ValueError('The parameter cannot contain special charaters(/,//,*,?) in the Boundry name')
            if(len(permission_policy_name) > 128):
