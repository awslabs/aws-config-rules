...
#
# Rule Name:
#   ECS_NO_PRIVILEGED_TASKS
# Description:
#   Check ECS Task Definitions for presence of privileged container definitions
#
# Rationale:
#   ECS Task Definitions should be configured without containers with elevated privileges.
#
# Indicative Severity:
#   Medium
#
...
#        And: It contains a parameter key other than TaskDefinition
#       Then: Return ERROR to specify valid key
#   Scenario: 2
#      Given: Rules parameter is provided. TaskDefinition is specified.
#        And: There is no value specified for TaskDefinition
#       Then: Return ERROR and print to Cloudwatch to specify valid TaskDefinitions
#   Scenario: 3
#      Given: Rules parameter is provided. TaskDefinition is specified
#        And: Task Definitions match TaskDefinitions (case sensitive) specified in values.
#       Then: Return COMPLIANT with annotation if TaskDefinitions have awslogs driver.
#   Scenario: 4
#      Given: Rules parameters are provided. TaskDefinition is specified
#      Given: Task Definitions match TaskDefinitions (case sensitive) specified in values.
#       Then: Return NON_COMPLIANT with annotation if TaskDefinitions have no awslogs driver.
#
import json
import sys
import datetime
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    taskdefs = []
    not_compliant = []
    privileged_container = "com.amazonaws.ecs.capability.privileged-container"
    ecs = get_client('ecs', event)
    if valid_rule_parameters:
        ...
        for taskdef in taskdefs:
            ...
            if "taskDefinition" not in response.keys():
                ...
                return None
            print(f'TaskDefinition: {response["taskDefinition"]}')
            for attribute in response["taskDefinition"]["requiresAttributes"]:
                if privileged_container == attribute["name"]:
                    evaluations.append(build_evaluation(taskdef, 'NON_COMPLIANT', event, annotation='Task Definition has privileged container.'))
                    not_compliant.append(taskdef)
                    break
            if taskdef not in not_compliant:
                evaluations.append(build_evaluation(taskdef, 'COMPLIANT', event, annotation='Task Definition does not have privileged container.'))
