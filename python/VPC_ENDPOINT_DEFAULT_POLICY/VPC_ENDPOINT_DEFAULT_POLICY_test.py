...
class ComplianceTest(unittest.TestCase):
    ...
    #  Scenario 2: At least one VPC is present and no interface endpoint is present on DescribeVpcEndpoints API -- GHERKIN Scenario 2
    def test_scenario_2(self):
        ...
        resp_expected = []
        response = RULE.lambda_handler(build_lambda_scheduled_event(), {})
        resp_expected.append(build_expected_response('NON_COMPLIANT', 'vpc-1234567', compliance_resource_type='AWS::EC2::VPC', annotation='There are no VPC interface endpoints present in '+ vpc_id+'.'))
        assert_successful_evaluation(self, response, resp_expected)
    ...
    def test_scenario_3(self):
        ...
        simulatecustompolicy_return = {
            'EvaluationResults': [
                ...
                {
                    'EvalActionName': 's3:PutObjectVersion',
                    'EvalResourceName': 'string',
                    'EvalDecision': 'allowed',
                }
            ]
        ...
        EC2_CLIENT_MOCK.describe_vpc_endpoints = MagicMock(return_value=describevpcendpoints_return)
        IAM_CLIENT_MOCK.simulate_custom_policy = MagicMock(return_value=simulatecustompolicy_return)
        response = RULE.lambda_handler(build_lambda_scheduled_event(), {})
        resp_expected.append(build_expected_response('NON_COMPLIANT', 'vpc-1234567', compliance_resource_type='AWS::EC2::VPC', annotation='The interface VPC endpoint (vpce-06e48c97f0bc5d501) is using the default open policy in VPC '+vpc_id+'.'))
        assert_successful_evaluation(self, response, resp_expected)
