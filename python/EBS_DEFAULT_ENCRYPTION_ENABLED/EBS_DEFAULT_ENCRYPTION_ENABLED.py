...
Parameters:
  ...
  |-----------------------|-----------|-----------------------------------------------|
  | KmsKeyId              | Optional  | Specific KMS key used to encrypt EBS volumes  |
  |                       |           | volumes by default, when blank any KMS key    |
...
Scenarios:
    Scenario 1:
      Given: the KmsKeyId is not specified.
        And: the EC2 EBS default encryption is not enabled
       Then: return NON_COMPLIANT
    Scenario 2:
      Given: the KmsKeyId is not specified.
        And: the EC2 EBS default encryption is enabled
        And: the encryption key is the AWS Managed alias/aws/ebs key
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    evaluation = None
    expected_kms_key = None
    ec2 = get_client('ec2', event)

    if 'KmsKeyId' in valid_rule_parameters:
        expected_kms_key = valid_rule_parameters['KmsKeyId']

    try:
        # response: 'EbsEncryptionByDefault': True|False
        response = ec2.get_ebs_encryption_by_default()
        if response['EbsEncryptionByDefault']:
            current_kms_key = ec2.get_ebs_default_kms_key_id()['KmsKeyId']
            if expected_kms_key and expected_kms_key != current_kms_key:
                annotation = f'Expected default key {expected_kms_key} but is currently {current_kms_key}'
                evaluation = build_evaluation(
                    event['accountId'], 'NON_COMPLIANT', event, annotation=annotation)
            else:
                annotation = f'EC2 EBS Encryption setting default KMS key is: {current_kms_key}'
                evaluation = build_evaluation(
                    event['accountId'], 'COMPLIANT', event, annotation=annotation)
        else:
            evaluation = build_evaluation(
                event['accountId'], 'NON_COMPLIANT', event, annotation=None)
