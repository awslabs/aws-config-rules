...
DEFAULT_RESOURCE_TYPE = 'AWS::EC2::NetworkAclEntry'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    # Initialize list of ports within security groups, to be used for NACL port evaluation
    security_group_ports = []

    # Defining non-compliant and compliant use cases
    compliant_annotation = 'Resource {} is COMPLIANT with all checks.'
    non_compliant_all_ports = 'Resource {} allows all ports within a rule.'
    non_compliant_port_range = 'Resource {} allows for port ranges in a given rule. Ensure that every rule should only enable one and only one port. Non-compliant value: {}'
    non_compliant_nacl_port = 'Network ACL {} has ports allowed that are not part of at least one security group. Non-compliant value: {}'
    non_compliant_ip_source = 'Resource {} allows all connections (0.0.0.0/0).'

    security_groups = fetch_all_items(ec2_client.describe_security_groups, 'SecurityGroups', 'NextToken')
    for security_group in security_groups:
        for permission in security_group['IpPermissions']:
            # Adding unique ports to our security group ports list to be used for NACL evaluations
            if permission.get('FromPort') and permission.get('FromPort') not in security_group_ports:
                security_group_ports.append(permission['FromPort'])

    nacls = fetch_all_items(ec2_client.describe_network_acls, 'NetworkAcls', 'NextToken')
    for nacl in nacls:
        for entry in nacl['Entries']:
            port_range = entry.get('PortRange')

            # Only check for ingress rules, if egress then continue to next
            if entry.get('Egress'):
                continue

            # Only check for allow actions, if deny then continue to next
            if entry.get('RuleAction') == 'deny':
                continue

            # If the CidrBlock is 0.0.0.0/0, that means the rule allows all IPs and should be non-compliant
            if entry['CidrBlock'] == '0.0.0.0/0':
                evaluations.append(build_evaluation(
                    nacl['NetworkAclId'], 'NON_COMPLIANT', event,
                    resource_type='AWS::EC2::NetworkAclEntry',
                    annotation=build_annotation(non_compliant_ip_source.format(nacl['NetworkAclId']))))
                continue

            if port_range:
                # If From does not exist, that means the rule is allowing all ports and should be non-compliant
                if not port_range.get('From'):
                    evaluations.append(build_evaluation(
                        nacl['NetworkAclId'], 'NON_COMPLIANT', event,
                        resource_type='AWS::EC2::NetworkAclEntry',
                        annotation=build_annotation(non_compliant_all_ports.format(nacl['NetworkAclId']))))
                    continue

                # If From does not equal To, that means the rule allows port ranges and should be non-compliant
                if port_range['From'] != port_range['To']:
                    annotation = non_compliant_port_range.format(
                        nacl['NetworkAclId'], f"From: {port_range['From']}, To: {port_range['To']}")
                    evaluations.append(build_evaluation(
                        nacl['NetworkAclId'], 'NON_COMPLIANT', event,
                        resource_type='AWS::EC2::NetworkAclEntry',
                ...
                # If the port in the NACL rule is not within any security group, it should be non_compliant
                if port_range['From'] not in security_group_ports:
                    evaluations.append(build_evaluation(
                        nacl['NetworkAclId'], 'NON_COMPLIANT', event,
                        resource_type='AWS::EC2::NetworkAclEntry',
                        annotation=build_annotation(non_compliant_nacl_port.format(nacl['NetworkAclId'], port_range['From']))))
                    continue

            # If the rule gets to this point, it has passed all evaluations and should be compliant
            annotation = compliant_annotation.format(nacl['NetworkAclId'])
            evaluations.append(build_evaluation(
                nacl['NetworkAclId'], 'COMPLIANT', event,
                resource_type='AWS::EC2::NetworkAclEntry',
                annotation=build_annotation(annotation)))

    # Return our list of evaluations if any non-compliant/compliant evaluations were determined
    if evaluations:
        return evaluations
