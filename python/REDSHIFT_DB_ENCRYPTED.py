...
Scenarios:
  Scenario: 1
     Given: No redshift clusters present
      Then: Return ERROR
   Scenario: 2 RedShift cluster is not encrypted
     Given: If the database is not encrypted
      Then: Return NON_COMPLIANT
   Scenario: 3 RedShift cluster is  encrypted
     Given: If the database is encrypted
...
def evaluate_compliance(event, configuration_item):
    ...
    for cluster_tuple in cluster_list:
        ...
        if not cluster_tuple[1]['db_encrypted']:
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='The database cluster is not encrypted'))
        else:
            evaluations.append(
                build_evaluation(cluster_tuple[0], 'COMPLIANT', event, annotation='The database cluster is encrypted'))
    return evaluations
