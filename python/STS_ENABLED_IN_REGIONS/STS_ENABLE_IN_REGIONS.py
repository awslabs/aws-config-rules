...
#####################################
Rule Name:
  STS_ENABLE_IN_REGIONS
Description:
  Verify that STS endpoints are enabled in whitelisted regions.
Trigger:
...
Rule Parameters:
  ActivateRegions
    Verify Security Token Service is activated in whitelisted region(s).
Scenarios:
  Scenario 1:
  Given: The same number of active and specified regions
    Then: Return COMPLIANT
  Scenario 2:
  Given: There are more active regions then specified.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    account_id = valid_rule_parameters["account_id"]
    role = valid_rule_parameters["role"]
    session = boto3.Session()
    region_list = session.get_available_regions("sts")
    active_region_list = valid_rule_parameters["active_regions"]
    compliant = "COMPLIANT"

    region = get_region_from_config_arn(event)
    active = check_region_active(region, account_id, role)
    if (active and not region in active_region_list) or (not active and (region in active_region_list)):
...
def check_region_active(region_name, account_id, role):
    sts = "sts."
    domain = ".amazonaws.com"
    session = boto3.Session(region_name=region_name)
    endpoint_url = f"{sts}{region_name}{domain}"
    client = session.client("sts", endpoint_url=endpoint_url)
    active = True

    try:
        client.assume_role(RoleArn='arn:aws:iam::' + account_id + ':role/' + role,
...
def lambda_handler(event, context):
    ...
    if not compliance_result:
        latest_evaluations.append(build_evaluation(event['accountId'], "NOT_APPLICABLE", event, resource_type='AWS::IAM::Role'))
