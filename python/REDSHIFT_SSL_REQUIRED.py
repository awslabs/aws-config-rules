...
Scenarios:
  ...
  Scenario: 2
   ...
   Scenario: 3
     Given: require_ssl is set to true
...
def evaluate_compliance(event, configuration_item):
    ...
    for cluster_tuple in cluster_list:
        ...
        print(type(cluster_tuple[1]))
        if cluster_tuple[1]['require_ssl'] == 'false':
            print(cluster_tuple[1])
            evaluations.append(build_evaluation(cluster_tuple[0], 'NON_COMPLIANT', event,
                                                annotation='SSL/TLS is not enforced for the cluster. Make sure to enable require_ssl by setting it to true.'))
        else:
            evaluations.append(build_evaluation(cluster_tuple[0], 'COMPLIANT', event, annotation='SSL/TLS is enforced'))
    return evaluations
